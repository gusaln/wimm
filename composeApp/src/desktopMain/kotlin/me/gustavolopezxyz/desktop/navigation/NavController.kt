...
class NavController(
    ...
    fun navigate(route: String, arguments: Map<String, String>? = null) {
        ...
        getKoin().logger.debug("Navigating to $entry, $arguments, stack size ${backStackScreens.size}")
    }
    ...
    fun navigateBack() {
        if (backStackScreens.isNotEmpty()) {
            ...
            backStackScreens.remove(currentScreen.value)
            getKoin().logger.debug("Navigating back to ${currentScreen.value}, ${currentScreen.value.arguments}, stack size ${backStackScreens.size}")
        }
    }
